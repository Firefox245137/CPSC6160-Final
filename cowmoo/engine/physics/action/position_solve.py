...
class PositionSolve: 
    def __init__(self): 
        ...
        self.name = "position_solve"          # Names are frequently useful 
    ...
    def act(self, data):                       # Run this action if the conditions are right 
        if self.condition_to_act(data):        # Check whether the conditions are right 
            for i in range(len(self.entity_state.position)):
                if(self.entity_state.active_particle[i]):
                    self.entity_state.position[i][0] += self.dt * self.entity_state.velocity[i][0]
                    self.entity_state.position[i][1] += self.dt * self.entity_state.velocity[i][1]
