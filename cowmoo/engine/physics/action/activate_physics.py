class ActivatePhysics: 
    def __init__(self, forces): 
        ...
        self.name = "activate_physics"    # Names are frequently useful 
    ...
    def act(self, data):                       # Run this action if the conditions are right 
        if self.condition_to_act(data):        # Check whether the conditions are right 
            if "active" in self.forces:
                self.entity_state.active_particle[data] = True
            if "gravity" in self.forces:
                self.entity_state.active_gravity[data] = True
            if "spring" in self.forces:
                self.entity_state.active_spring[data] = True
            if "drag" in self.forces:
                self.entity_state.active_drag[data] = True
