...
class GravityForceAction: 
    def __init__(self): 
        ...
        self.name = "gravity_force_action"          # Names are frequently useful 
    ...
    def act(self, data):                       # Run this action if the conditions are right 
        if self.condition_to_act(data):        # Check whether the conditions are right 
            for i in range(0, len(data.acceleration)):
                if data.active_particle[i] and data.active_gravity[i]:
                    data.acceleration[i][0] += self.entity_state.gravity[0]
                    data.acceleration[i][1] += self.entity_state.gravity[1]
